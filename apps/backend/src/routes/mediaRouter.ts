...
// // GET route for fetching all Media associated with a certain creator
// MediaRouter.get('/bycreator', requireAuth, async (req, res) => {
//     const currentUser = req.session!.user;

//     try {
//         const media = await MediaModel.find({ creator: currentUser });
//         res.status(200).send(media);
//     } catch (error) {
...
// // GET route for fetching media by creator and type
// MediaRouter.get('/bycreator/:creator/type/:type', requireAuth, async (req, res) => {
//     const { creator, type } = req.params;
//     //const sanitizedType = type.trim().slice(0, -2);

//     try {
//         const media = await MediaModel.find({ creator, type });
//         res.status(200).send(media);
//     } catch (error) {
//         res.status(500).json({ message: 'Error fetching media by creator and type: ', error: error.message });
//     }
// });

// // GET route for fetching media by creator and rating
// MediaRouter.get('/bycreator/:creator/rating/:rating', requireAuth, async (req, res) => {
//     const { creator, rating } = req.params;

//     try {
//         const media = await MediaModel.find({ creator, rating });
//         res.status(200).send(media);
//     } catch (error) {
//         res.status(500).json({ message: 'Error fetching media by creator and rating: ', error: error.message });
//     }
// });
